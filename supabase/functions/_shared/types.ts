...
export interface GameState {
  ...
  headUsed?: number;
  /**
   * Короткие нарды, домашнее правило: индексы пунктов, куда в ТЕКУЩЕМ ходу была
   * побита шашка соперника В СВОЁМ ДОМЕ. Побившая шашка с такого пункта больше
   * не может ходить обычным ходом до конца хода — разрешён только ВЫНОС ('off').
   * Сбрасывается в endTurn/startTurn.
   */
  hitLock?: number[];
}
