 ...
 * в правый-низ. Слоты доски позиционно симметричны сверху/снизу, поэтому фишки
 * ложатся точно на резные места board.png, а сами фишки/цифры НЕ переворачиваем
 * (отражаем только координаты, не элементы).
 * ========================================================================== */
...
export function pointGeom(index: number, viewer: Color = 'w'): PointGeom {
  ...
  let baseY: number;
  let dir: 1 | -1;
  if (row === 'top') { baseY = TOP_RAIL; dir = 1; }
  else { baseY = BOT_RAIL; dir = -1; }

  // Перспектива чёрного — отражаем по вертикали (дом чёрных приходит в низ-право).
  if (viewer === 'b') {
    baseY = reflectY(baseY);
    dir = dir === 1 ? -1 : 1;
    row = row === 'top' ? 'bottom' : 'top';
  }
  return { index, cx, baseY, dir, row };
}
